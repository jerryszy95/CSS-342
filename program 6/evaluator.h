...
public:
    Evaluator(const string &s) : str(s) { // constructor receives an exp
        ...
        PREC_TABLE[26].inputSymbol = 21;    // EXP
        PREC_TABLE[26].topOfStack = 22;
        
        PREC_TABLE[27].inputSymbol = 1;    // ASSIGN
        PREC_TABLE[27].topOfStack =  2;
...
private:
    Precedence PREC_TABLE[28];                // precedence table
    vector<TokenType> opStack;                // Operator stack for conversion
    vector<NumericType> postFixStack;         // Postfix machine stack
    vector<NumericType> postFixVarStack;      // Postfix machine stack for variable
    istringstream str;                        // The character stream
    
    // Internal routines
    NumericType getTop();                         // Get top of postfix stack
    void binaryoperator(TokenType topOp);         // process an operator
    void unaryoperator(TokenType topOp);          // process an unary operator
